 ...
 */
function notifyReturnRequestApproved(PDO $db, string $userEmail, string $bookTitle, string $isbn) {
    $message = "Your return request for '$bookTitle' (ISBN: $isbn) has been approved. Thank you for returning the book on time.";
 ...
 */
function notifyAdditionRequestApproved(PDO $db, string $userEmail, string $bookTitle, string $isbn) {
    $message = "Your book addition request for '$bookTitle' (ISBN: $isbn) has been approved and added to the library collection. Thank you for your suggestion!";
 ...
 */
function notifyReservationQueueUpdate(PDO $db, string $userEmail, string $bookTitle, string $isbn, int $newPosition) {
    $message = "Your reservation position for '$bookTitle' (ISBN: $isbn) has been updated. You are now #$newPosition in the queue.";
    return createNotification($db, $userEmail, $message, 'ReservationQueueUpdate');
}

/**
 * Notify user when book due date is approaching (1 day before)
 */
function notifyDueDateApproaching(PDO $db, string $userEmail, string $bookTitle, string $isbn, string $dueDate) {
    $dueDateFormatted = (new DateTime($dueDate))->format('M d, Y');
    $message = "Reminder: Your borrowed book '$bookTitle' (ISBN: $isbn) is due tomorrow ($dueDateFormatted). Please return it to avoid late fees.";
...
function sendDueDateReminders(PDO $db): int {
    ...
    foreach ($books as $book) {
        if (notifyDueDateApproaching($db, $book['requester_email'], $book['title'], $book['isbn'], $book['due_date'])) {
            $count++;
        }
    }
...
function getUserNotifications(PDO $db, string $userEmail, int $limit = 50): array {
    ...
    // Add isRead field (default all to false since we don't track read status in DB)
...
function notifyQueuePositionChanges(PDO $db, string $isbn): void {
    ...
    foreach ($reservations as $reservation) {
        ...
        if (!$checkStmt->fetch()) {
            notifyReservationQueueUpdate($db, $reservation['user_email'], $bookTitle, $isbn, $reservation['queue_position']);
        }
    }
}
?>
