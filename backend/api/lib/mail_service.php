...
class MailService
{
    ...
    {
        // Prefer backend/logs for cross-platform compatibility (Windows/Linux).
        ...
        if (is_dir($logsDir) && is_writable($logsDir)) {
            return rtrim($logsDir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'iit_shelf_otp.log';
        }

        // Fallback to system temp dir if project logs dir is not writable.
        return rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'iit_shelf_otp.log';
    }
    ...
    public static function logOtpToFile($email, $otp, $purpose)
    {
        ...
        if (@file_put_contents($logFile, $logEntry, FILE_APPEND) === false) {
            error_log('Unable to write OTP log file: ' . $logFile);
        }
    }
