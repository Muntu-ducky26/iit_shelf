...
class OtpAttemptTracker
{
    ...
    private static function getLogFilePath(): string
    {
        ...
        if (is_dir($logsDir) && is_writable($logsDir)) {
            return rtrim($logsDir, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'iit_shelf_otp_attempts.log';
        }

        return rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'iit_shelf_otp_attempts.log';
    }
    ...
    public static function logAttempt(string $email, string $purpose, bool $success, ?string $reason = null): void
    {
        ...
        if (@file_put_contents($logFile, $entry, FILE_APPEND) === false) {
            error_log('Unable to write OTP attempts log file: ' . $logFile);
        }
    }
